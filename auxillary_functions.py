...
def metropolis_filter_simple(current_state, proposal_state, lattice_basis):
    """
    ...
    :return: Bool for accept or reject proposal step
    """
    alpha = min(1, (lattice_gaussian(lattice_basis, current_state, proposal_state)))
    if np.random.uniform(0, 1) <= alpha:
        return True
    else:
        return False
...
def lattice_gaussian(lattice_basis, old_vector, new_vector):
    ...
    factor = -(np.linalg.norm(np.dot(lattice_basis.T, new_vector))**2
               - np.linalg.norm(np.dot(lattice_basis.T, old_vector))**2)/2
    try:
        ans = math.exp(factor)
